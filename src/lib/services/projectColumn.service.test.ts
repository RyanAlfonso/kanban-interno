...
} from './projectColumn.service'; // Adjust path as necessary
import prisma from '../prismadb'; // Actual prisma client
...
describe('ProjectColumn Service', () => {
  ...
  describe('createProjectColumn', () => {
    ...
    it('should create a new project column successfully', async () => {
      prismaMock.project.findUnique.mockResolvedValueOnce({ id: 'proj1', name: 'Test Project', description: null, createdAt: new Date(), updatedAt: new Date() });
      ...
      expect(prismaMock.project.findUnique).toHaveBeenCalledWith({ where: { id: 'proj1' } });
      expect(prismaMock.projectColumn.create).toHaveBeenCalledWith({ data: columnData });
    });
  ...
  describe('updateProjectColumn', () => {
    ...
    it('should update a column successfully', async () => {
      const existingColumn = { ...mockProjectColumn, id: columnId, projectId: 'proj1', name: 'Old Name' };
      ...
      expect(prismaMock.projectColumn.findUnique).toHaveBeenCalledWith({ where: { id: columnId } });
    ...
    it('should throw error if updated name conflicts with another column in the same project', async () => {
        const existingColumn = { ...mockProjectColumn, id: columnId, projectId: 'proj1', name: 'Old Name' };
        const conflictingColumn = { ...mockProjectColumn, id: 'col2', projectId: 'proj1', name: 'Updated Name' };

        prismaMock.projectColumn.findUnique.mockResolvedValueOnce(existingColumn); // For initial find
        prismaMock.projectColumn.findUnique.mockResolvedValueOnce(conflictingColumn); // For conflicting name check

        await expect(updateProjectColumn(columnId, { name: 'Updated Name' })).rejects.toThrow(
  ...
  describe('deleteProjectColumn', () => {
    ...
    it('should delete a column successfully', async () => {
      ...
      expect(prismaMock.projectColumn.findUnique).toHaveBeenCalledWith({ where: { id: columnId } });
      expect(prismaMock.projectColumn.delete).toHaveBeenCalledWith({ where: { id: columnId } });
    });
  ...
  describe('reorderProjectColumns', () => {
    ...
    const columnsInDb = [
        { ...mockProjectColumn, id: 'col2', order: 0, projectId }, // Intentionally different order
        { ...mockProjectColumn, id: 'col1', order: 1, projectId },
    ];
     const reorderedColumnsMock = [ // What we expect after reordering
    ...
    it('should reorder columns successfully', async () => {
      prismaMock.projectColumn.findMany
        .mockResolvedValueOnce(columnsInDb.map(c => ({ id: c.id, order: c.order }))) // For validation
        .mockResolvedValueOnce(reorderedColumnsMock); // For final return

      (prismaMock.$transaction as jest.Mock).mockImplementation(async (promises) => {
        // Simulate successful transaction by resolving all promises
        return Promise.all(promises.map((p: any) => p));
      });

      // Mock individual updates within the transaction
      ...
      // Check if update was called for each column with correct new order
      expect(prismaMock.projectColumn.update).toHaveBeenCalledWith({ where: { id: 'col1' }, data: { order: 0 } });
      expect(prismaMock.projectColumn.update).toHaveBeenCalledWith({ where: { id: 'col2' }, data: { order: 1 } });

      expect(result).toEqual(reorderedColumnsMock);
      expect(prismaMock.projectColumn.findMany).toHaveBeenLastCalledWith({ // The final fetch
         where: { projectId: projectId },
         orderBy: { order: 'asc' },
      });
    });
    ...
    it('should throw error if column IDs are invalid or counts mismatch', async () => {
      prismaMock.projectColumn.findMany.mockResolvedValueOnce(columnsInDb.map(c => ({ id: c.id, order: c.order })));

      await expect(reorderProjectColumns(projectId, ['col1', 'col3'])).rejects.toThrow( // col3 is invalid
    ...
    it('should throw error if transaction fails', async () => {
        ...
        (prismaMock.$transaction as jest.Mock).mockRejectedValueOnce(new Error("Transaction failed"));

        await expect(reorderProjectColumns(projectId, orderedIds)).rejects.toThrow(
            'Failed to reorder columns. Please try again.'
        );
    });
  });
});
