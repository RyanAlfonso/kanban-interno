...
export async function GET(
  ...
  try {
    ...
    logger.error(
      `Error fetching columns for project ${params.projectId}:`,
      error
    );
    if (error instanceof Error && error.message.includes("not found")) {
      // Or specific error types from service
...
export async function POST(
  ...
  try {
    ...
    if (session.user.role !== "ADMIN") {
      return new NextResponse("Forbidden: User is not an Admin", {
        status: 403,
      });
    }
    ...
    if (!project) {
      return new NextResponse(`Project with ID ${projectId} not found.`, {
        status: 404,
      });
    }
    ...
    logger.error(
      `Error creating column for project ${params.projectId}:`,
      error
    );
